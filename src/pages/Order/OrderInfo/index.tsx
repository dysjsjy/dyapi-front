      ...
      if (isMobile) {
        ...
        setTimeout(() => {
          history.push(
            `/order/pay/${
              record.id
            }?codeUrl=${record?.codeUrl?.trim()}&payType=${record?.payType?.trim()}`,
          );
        }, 800);
      }
    } else {
    ...
    <Spin spinning={loading}>
      ...
      >
        <Descriptions>
          ...
          <Descriptions.Item key={'addPoints'} label="增加积分数量 (个)">
            {data?.addPoints}
          </Descriptions.Item>
          <Descriptions.Item key={'payType'} label="支付类型">
            <Tag color={orderPayTypeEnum[data?.payType ?? 'default']}>{data?.payType}</Tag>
          </Descriptions.Item>
      ...
      <ProCard style={{ marginTop: 20 }} layout={'center'}>
        <span>
          ...
          <a
            target={'_blank'}
            href={
              'https://gitee.com/wangkeyaowky/api-interface-documentation/blob/master/README.md#%E4%B8%83%E5%85%8D%E8%B4%A3%E5%A3%B0%E6%98%8E%E6%9F%92%E6%9C%A8%E6%8E%A5%E5%8F%A3%E7%94%A8%E6%88%B7%E5%8D%8F%E8%AE%AE'
            }
            rel="noreferrer"
          >
            {' '}
